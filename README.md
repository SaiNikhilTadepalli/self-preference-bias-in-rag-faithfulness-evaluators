...
All experiments were conducted on a subset of $N=100$ randomly sampled QA pairs from the BioASQ dataset.
...
* **Weaknesses in controlled corruption:** Corruptions are generated by the same model, Mistral, that produced the original LLM answers. Mistral therefore may be better at generating corruptions that are detectable in human-style text than in its own outputs, either because it has stronger representations of what makes its own claims coherent, or because human-authored answers contain more natural-language hedging that makes entity substitutions slightly more conspicuous. Whilst this is hard to rule out without a third-party corruption generator, qualitative inspection of the generated corruption data suggests the corruptions are indeed purely factual and do not affect stylistic signature, especially so with entity substitution. Further experiments using a different model family were not performed due to time constraints and API credit limits.
