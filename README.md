...
* **Weaknesses in controlled corruption:** The most pressing limitation is that the corruptions are generated by the same model, Mistral, that produced the original LLM answers. Mistral therefore may be better at generating corruptions that are detectable in human-style text than in its own outputs, either because it has stronger representations of what makes its own claims coherent, or because human-authored answers contain more natural-language hedging that makes entity substitutions slightly more conspicuous. Whilst this is hard to rule out without a third-party corruption generator, qualitative inspection of the generated corruption data suggests the corruptions are indeed purely factual and do not affect stylistic signature, especially so with entity substitution. 
